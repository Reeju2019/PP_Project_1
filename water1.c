...
int main()
{
    ...
    // #pragma omp parallel for schedule(static, 10000) private(j)
    // #pragma omp parallel for schedule(dynamic) private(j)
    // #pragma omp parallel for schedule(dynamic, 1) private(j)
    // #pragma omp parallel for schedule(dynamic, 100) private(j)
    // #pragma omp parallel for schedule(dynamic, 10000) private(j)
    // #pragma omp parallel for schedule(guided) private(j)
    // #pragma omp parallel for schedule(guided, 1) private(j)
    // #pragma omp parallel for schedule(guided, 100) private(j)
    // #pragma omp parallel for schedule(guided, 10000) private(j)
#pragma omp parallel for schedule(auto) private(j)
    for (i = 0; i < nmol - 1; i++)
    {
        ...
        printf("Thread %d evaluated %d pairs\n", omp_get_thread_num(), npair);
        // Print npair
    }

    // Print npair_array counts after loop
